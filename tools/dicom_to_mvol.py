...
Header layout MUST stay in lock-step with src/volume/volume_file.h:
...
Usage: python tools/dicom_to_mvol.py [dicom_dir] [out.mvol]
"""
import glob
import os
import struct
...
DICOM_DIR = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "assets", "dicom")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "assets", "volume.mvol")
