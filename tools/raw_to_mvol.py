...
src/volume/volume_file.h), so the browser renders REAL anatomy instead of the
synthetic phantom. uint8 source -> R8; uint16 source -> packed RG8.

Usage:
  python tools/raw_to_mvol.py <in.raw> <W> <H> <D> <uint8|uint16> [out.mvol] [max_dim]
"""
import os
import struct
import sys
...
def main():
    ...
    out = sys.argv[6] if len(sys.argv) > 6 else os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "assets", "volume.mvol")
