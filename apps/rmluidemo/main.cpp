...
static void onAdapter(WGPURequestAdapterStatus st, WGPUAdapter a,
                      WGPUStringView msg, void* ud1, void*) {
    auto* r = static_cast<AdapterReq*>(ud1);
    if (st == WGPURequestAdapterStatus_Success) r->adapter = a;
    else SDL_Log("RequestAdapter failed: %.*s", (int)msg.length, msg.data ? msg.data : "");
    r->done = true;
}

static void onDevice(WGPURequestDeviceStatus st, WGPUDevice d,
                     WGPUStringView msg, void* ud1, void*) {
    auto* r = static_cast<DeviceReq*>(ud1);
    if (st == WGPURequestDeviceStatus_Success) r->device = d;
...
static void onUncapturedError(WGPUDevice const*, WGPUErrorType type,
    ...
    SDL_Log("[WGPU error type=%d]: %.*s", (int)type, (int)msg.length,
            msg.data ? msg.data : "");
}
...
static bool pumpUntil(WGPUInstance inst, bool& flag, const char* op,
    ...
    while (!flag) {
        ...
        if (SDL_GetTicks() - start > timeoutMs) {
            SDL_Log("%s timed out after %llu ms", op,
...
static WGPUSurface createSurface(WGPUInstance instance, SDL_Window* window) {
    ...
    hwndDesc.hinstance   = SDL_GetPointerProperty(
        SDL_GetWindowProperties(window),
        SDL_PROP_WINDOW_WIN32_INSTANCE_POINTER, nullptr);
    hwndDesc.hwnd        = SDL_GetPointerProperty(
        SDL_GetWindowProperties(window),
    ...
    uint64_t xwindow  = static_cast<uint64_t>(SDL_GetNumberProperty(
        SDL_GetWindowProperties(window),
...
public:
    void set_device(WGPUDevice dev, WGPUQueue q) { device_ = dev; queue_ = q; }
                        ...
                        Rml::TextureHandle         texture) override
    {
        ...
        if (scissor_enabled_) {
            dl->PushClipRect(
                ImVec2(static_cast<float>(scissor_.Left()),
                       static_cast<float>(scissor_.Top())),
                ImVec2(static_cast<float>(scissor_.Right()),
        ...
        if (!use_white)
            dl->PushTextureID(static_cast<ImTextureID>(texture));
        ...
        for (const Rml::Vertex& v : geo.vertices) {
            dl->_VtxWritePtr->pos =
                ImVec2(v.position.x + translation.x,
                       v.position.y + translation.y);
            dl->_VtxWritePtr->uv  =
                use_white ? white_uv : ImVec2(v.tex_coord.x, v.tex_coord.y);
            dl->_VtxWritePtr->col =
                IM_COL32(v.colour.red, v.colour.green,
        ...
        Rml::Vector2i              source_dimensions) override
    {
        ...
        const size_t upload_size =
            static_cast<size_t>(aligned_bpr) * h;
        wgpuQueueWriteTexture(queue_, &dst, upload_data, upload_size,
                              &layout, &extent);
    ...
    void ReleaseTexture(Rml::TextureHandle handle) override {
        ...
        if (tex)  { wgpuTextureDestroy(tex); wgpuTextureRelease(tex); }
        textures_.erase(it);
    }
...
int main(int, char**) {
    ...
    const WGPUTextureFormat surfaceFormat =
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            switch (event.type) {
                ...
                case SDL_EVENT_MOUSE_MOTION:
                    input_state.on_mouse_move(event.motion.x, event.motion.y,
        ...
        WGPURenderPassEncoder rp =
            wgpuCommandEncoderBeginRenderPass(enc, &passDesc);
