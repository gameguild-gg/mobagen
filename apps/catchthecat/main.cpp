...
// ============================================================
static void rebuildECS(ecs::World& ecsWorld, const CatWorld& catWorld,
                       std::vector<ecs::Entity>& cells, ecs::Entity& catEntity) {
...
static void syncECS(ecs::World& ecsWorld, const CatWorld& catWorld,
                    const std::vector<ecs::Entity>& cells, ecs::Entity catEntity) {
  const auto& state = catWorld.worldState();
  for (std::size_t i = 0; i < cells.size(); ++i)
    ecsWorld.get<BlockedState>(cells[i]).blocked = state[i];
  if (ecsWorld.valid(catEntity))
    ecsWorld.get<AgentState>(catEntity).pos = catWorld.getCat();
}
...
static int parseCommandLineArguments(int argc, char** argv, GameConfig& config) {
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "--headless") {
      ...
      if (turn == "cat")          config.isCatTurn = true;
      else if (turn == "catcher") config.isCatTurn = false;
      else {
...
static int runRegularMode(int size) {
  ...
  SDL_Window* window = SDL_CreateWindow("Catch The Cat", wgpu_surface_width, wgpu_surface_height,
  ...
  while (!done) {
    ...
    // Settings panel (equivalent to original World::OnGui, context-param removed)
    {
      ...
      if (ImGui::Button("Step")) { catWorld.setSimulating(false); catWorld.step(); }
      ImGui::SameLine();
