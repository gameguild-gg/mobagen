...
namespace mobagen::compositions {
  ...
  ProjectStartupResult prepare_and_open_project(
    ...
    result.bootstrap = bootstrap_project(
        manifest_path, std::move(options), services.http_client
    );
    if (!result.bootstrap.ok()) return result;
    result.project = open_locked_project(
        manifest_path, std::move(locked_options), services.modules
    );
    return result;
  }
