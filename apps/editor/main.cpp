...
// --- Dawn: synchronous adapter request ---
...
// --- Dawn: synchronous device request ---
...
// --- Platform-specific WebGPU surface creation (native only) ---
...
int main(int, char**) {
  ...
  SDL_Window* window = SDL_CreateWindow("Editor", wgpu_surface_width, wgpu_surface_height, SDL_WINDOW_RESIZABLE);
  if (!window) {
  ...
  while (!done) {
    ...
    if (w != wgpu_surface_width || h != wgpu_surface_height)
      ResizeSurface(w, h);
    ...
    wgpuDeviceTick(wgpu_device);  // Dawn: flush validation messages
#endif
