...
static inline void integrate(Position& p, const Velocity& v) {
    ...
    for (int k = 0; k < 32; ++k) {  // synthetic CPU work so the system is compute-bound
        x += v.x * dt; y += v.y * dt; z += v.z * dt; dt *= 0.999f;
    }
    p.x = x; p.y = y; p.z = z;
}
...
int main() {
    ...
    World ws; populate(ws, N);   // serial world
    World wp; populate(wp, N);   // parallel world (identical init)
    ...
    jobs::WaitGroup wg;
    sched.parallel_for(n, grain, [&](std::size_t lo, std::size_t hi) {
        wp.apply_range<Position, Velocity>(lo, hi,
            [](Entity, Position& p, Velocity& v) { integrate(p, v); });
    }, wg);
    sched.wait(wg);
