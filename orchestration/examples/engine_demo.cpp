...
#include "event_bus.hpp"        // core/messaging
#include "reactive.hpp"         // core/reactive
#include "scheduler.hpp"        // core/jobs
#include "transform.hpp"        // core/scene
#include "transform_system.hpp"
#include "world.hpp"            // core/ecs

#include <glm/gtc/quaternion.hpp>
#include <cstdio>

struct Spin { float speed; };               // a sim component
struct VolumeLoaded { int entities; };      // a coarse event
...
int main() {
    ...
    int lut_uploads = 0;
    reactive::Effect upload_lut([&] { (void)lut_key.get(); ++lut_uploads; });
    ...
    auto tick = [&](float dt) {
        ...
        sched.parallel_for(n, 4096, [&](std::size_t lo, std::size_t hi) {
            world.apply_range<scene::Transform, Spin>(lo, hi,
                [&](ecs::Entity, scene::Transform& t, Spin& s) {
                    t.rotation = glm::angleAxis(s.speed * dt, glm::vec3(0, 1, 0)) * t.rotation;
                });
        }, wg);
        sched.wait(wg);
