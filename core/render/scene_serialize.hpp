...
template <class T>
bool take(const std::uint8_t*& p, const std::uint8_t* end, T& out) {
...
inline std::vector<std::uint8_t> save_scene(ecs::World& world) {
    ...
    for (ecs::Entity e : order) {
        ...
        put(b, t.position.x); put(b, t.position.y); put(b, t.position.z);
        put(b, t.rotation.x); put(b, t.rotation.y); put(b, t.rotation.z); put(b, t.rotation.w);
        put(b, t.scale.x); put(b, t.scale.y); put(b, t.scale.z);
        ...
        if (hasVol) {
            ...
            put(b, v.source.id);
            put(b, v.source.width); put(b, v.source.height); put(b, v.source.depth);
            put(b, v.source.spacing_mm.x); put(b, v.source.spacing_mm.y); put(b, v.source.spacing_mm.z);
            put(b, static_cast<std::uint8_t>(v.source.format));
            put(b, v.display.window_center); put(b, v.display.window_width);
...
inline std::vector<ecs::Entity> load_scene(ecs::World& world,
    ...
    struct Node { scene::Transform t; std::int32_t parentIdx = -1; bool hasVol = false; VolumeRenderable vol; };
    std::vector<Node> nodes(count);
    for (std::uint32_t i = 0; i < count; ++i) {
        Node& nd = nodes[i];
        scene::Transform& t = nd.t;
        if (!take(p, end, t.position.x) || !take(p, end, t.position.y) || !take(p, end, t.position.z)) return {};
        if (!take(p, end, t.rotation.x) || !take(p, end, t.rotation.y) ||
            !take(p, end, t.rotation.z) || !take(p, end, t.rotation.w)) return {};
        ...
        if (nd.hasVol) {
            ...
            if (!take(p, end, v.source.id) ||
                !take(p, end, v.source.width) || !take(p, end, v.source.height) || !take(p, end, v.source.depth) ||
                !take(p, end, v.source.spacing_mm.x) || !take(p, end, v.source.spacing_mm.y) ||
                !take(p, end, v.source.spacing_mm.z) ||
                !take(p, end, fmt) ||
                !take(p, end, v.display.window_center) || !take(p, end, v.display.window_width) ||
                !take(p, end, v.display.transfer_preset) ||
                !take(p, end, mode) ||
                !take(p, end, v.display.iso_threshold)) return {};
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        Node& nd = nodes[i];
        nd.t.parent = (nd.parentIdx >= 0 && nd.parentIdx < static_cast<std::int32_t>(count))
                          ? created[nd.parentIdx] : ecs::kInvalidEntity;
        nd.t.dirty = true;
