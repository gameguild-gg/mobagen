...
namespace detail {
inline std::size_t next_event_id() { static std::size_t c = 0; return c++; }
template <class E> std::size_t event_id() { static const std::size_t id = next_event_id(); return id; }
}  // namespace detail
...
public:
    ...
    void unsubscribe(Id id) {
        ...
        for (auto it = subs.begin(); it != subs.end(); ++it)
            if (it->first == id) { subs.erase(it); return; }
    }

    template <class E>
    void post(E e) { channel<E>().queue.push_back(std::move(e)); }

    void process() { for (auto& c : channels_) if (c) c->drain(); }   // drain all types
    template <class E> void process() { channel<E>().drain(); }       // drain one type

private:
    struct IChannel { virtual ~IChannel() = default; virtual void drain() = 0; };
