...
struct FramePool {
    ...
    void release(void* p, std::size_t n) {
        if (n <= kBlock) free_.push_back(p);  // keep for reuse
        else std::free(p);
    }
    ~FramePool() { for (void* p : free_) std::free(p); }
};
...
struct Task {
    struct promise_type {
        ...
        static void operator delete(void* p, std::size_t n) noexcept {
            detail::t_frame_pool.release(p, n);
        }
    ...
    handle_t release() { auto h = handle; handle = {}; return h; }
};
