...
template <class Fn>
inline Task make_range_task(Fn fn, std::size_t begin, std::size_t end) {
    fn(begin, end);
    co_return;
}
...
public:
    ...
    template <class Fn>
    void parallel_for(std::size_t n, std::size_t grain, Fn fn, WaitGroup& wg) {
        if (grain == 0) grain = 1;
...
private:
    ...
    struct Worker { ChaseLevDeque<kDequeCap> q; };
