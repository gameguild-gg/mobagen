...
namespace mobagen::plugins {
  namespace {
    ...
    constexpr std::uint32_t align_command(std::uint32_t size) noexcept {
      return (size + MOBAGEN_WASM_COMMAND_ALIGNMENT - 1U)
    ...
    void add_issue(Result& result, WasmAssetStoreIssueCode code, std::string message) {
      result.issues.push_back({code, std::move(message), {}});
    }

    bool valid_status(std::uint32_t status) noexcept {
      return status <= MOBAGEN_WASM_STATUS_FAILED;
    }
    ...
    bool decode_response(
      ...
      if (!byte_size.has_value() || !opcode.has_value() || !request_id.has_value()
          || !status.has_value() || !valid_status(*status) || *byte_size == 0
          || *byte_size % MOBAGEN_WASM_COMMAND_ALIGNMENT != 0
          || offset > bytes.size() || *byte_size > bytes.size() - offset) {
        return false;
      }
      ...
      if (*opcode != MOBAGEN_WASM_ASSET_RESULT_VIEW
          || *byte_size < MOBAGEN_WASM_ASSET_VIEW_RESULT_V1_SIZE) {
        return false;
      }
      ...
      if (!index.has_value() || !generation.has_value() || !payload_size.has_value()
          || !reserved.has_value() || *reserved != 0) {
        return false;
      }
      const auto expected_size =
          static_cast<std::uint64_t>(MOBAGEN_WASM_ASSET_VIEW_RESULT_V1_SIZE)
          + *payload_size;
      if (expected_size > std::numeric_limits<std::uint32_t>::max()
          || align_command(static_cast<std::uint32_t>(expected_size)) != *byte_size) {
        return false;
      }
      response.handle = {*index, *generation};
      response.payload = bytes.subspan(
          offset + MOBAGEN_WASM_ASSET_VIEW_RESULT_V1_SIZE, *payload_size
      );
      return true;
    }
  ...
  WasmAssetConfigurationResult build_wasm_asset_configuration(
    ...
    if (assets.size() > MOBAGEN_WASM_ASSET_MAX_CONFIGURED_ASSETS) {
      add_issue(
          result, WasmAssetStoreIssueCode::InvalidConfiguration,
          "portable asset configuration exceeds the asset-count limit"
      );
      return result;
    }
    ...
    for (const auto& asset : assets) {
      if (asset.payload.size() > MOBAGEN_WASM_ASSET_MAX_CONFIGURED_BYTES) {
        add_issue(
            result, WasmAssetStoreIssueCode::InvalidConfiguration,
            "portable asset configuration exceeds the byte limit"
        );
        return result;
      }
      payload_total += asset.payload.size();
      total += align_command(
          MOBAGEN_WASM_ASSET_CONFIGURATION_ENTRY_V1_SIZE
          + static_cast<std::uint32_t>(asset.payload.size())
      );
      if (payload_total > MOBAGEN_WASM_ASSET_MAX_CONFIGURED_BYTES
          || total > MOBAGEN_WASM_MAX_CONFIGURATION_BYTES) {
        add_issue(
            result, WasmAssetStoreIssueCode::InvalidConfiguration,
            "portable asset configuration exceeds the byte limit"
        );
    ...
    try {
      ...
      add_issue(
          result, WasmAssetStoreIssueCode::OutOfMemory,
          "portable asset configuration allocation failed"
      );
      return result;
    }
    ...
    for (const auto& asset : assets) {
      ...
      write_u32(
          result.bytes, cursor + MOBAGEN_WASM_ASSET_ID_V1_SIZE,
          static_cast<std::uint32_t>(asset.payload.size())
      );
      write_u32(result.bytes, cursor + MOBAGEN_WASM_ASSET_ID_V1_SIZE + 4, 0);
      std::ranges::copy(
          asset.payload,
          result.bytes.begin() + cursor + MOBAGEN_WASM_ASSET_CONFIGURATION_ENTRY_V1_SIZE
      );
      cursor += align_command(
          MOBAGEN_WASM_ASSET_CONFIGURATION_ENTRY_V1_SIZE
          + static_cast<std::uint32_t>(asset.payload.size())
      );
    }
  ...
  bool WasmAssetResponseBatchView::next(
  ...
  WasmAssetCommandBuffer::WasmAssetCommandBuffer(
      std::uint32_t max_commands, std::uint32_t byte_capacity
  )
      : max_commands_(max_commands), byte_capacity_(byte_capacity) {
    if (max_commands == 0 || max_commands > MOBAGEN_WASM_MAX_COMMANDS_PER_BATCH
        || byte_capacity == 0
  ...
  bool WasmAssetCommandBuffer::queue_acquire(
      std::uint32_t request_id, const MobagenWasmAssetIdV1& id
  ) noexcept {
    if (command_count_ == max_commands_
        || bytes_.size() + MOBAGEN_WASM_ASSET_ACQUIRE_COMMAND_V1_SIZE
               > byte_capacity_) {
      return false;
    }
  ...
  bool WasmAssetCommandBuffer::queue_handle(
      std::uint32_t opcode, std::uint32_t request_id, MobagenWasmHandle32 handle
  ) noexcept {
    if (command_count_ == max_commands_
        || bytes_.size() + MOBAGEN_WASM_ASSET_HANDLE_COMMAND_V1_SIZE
               > byte_capacity_) {
      return false;
    }
    const auto offset = bytes_.size();
  ...
  WasmAssetStoreProcessResult WasmAssetCommandBuffer::process(
      WasmCommandChannel& channel
  ) const {
    WasmAssetStoreProcessResult result;
    if (command_count_ == 0) {
      add_issue(
          result, WasmAssetStoreIssueCode::InvalidCapacity,
          "portable asset command batch is empty"
      );
      return result;
    }
    ...
    if (output.command_count != command_count_) {
      add_issue(
          result, WasmAssetStoreIssueCode::InvalidResponse,
          "portable asset provider returned the wrong response count"
      );
      return result;
    }
    ...
    for (std::uint32_t index = 0; index < command_count_; ++index) {
      ...
      WasmAssetResponseView response;
      if (!input_size.has_value() || !input_opcode.has_value()
          || !input_request.has_value()
          || !decode_response(output.bytes, output_cursor, response)) {
        add_issue(
            result, WasmAssetStoreIssueCode::InvalidResponse,
            "portable asset provider returned a malformed response"
        );
        return result;
      }
      const auto expected_opcode = expected_result_opcode(*input_opcode);
      if (!expected_opcode.has_value() || response.opcode != *expected_opcode
          || response.request_id != *input_request) {
        add_issue(
            result, WasmAssetStoreIssueCode::MismatchedResponse,
            "portable asset response does not match its request"
        );
        return result;
      }
    ...
    if (input_cursor != bytes_.size() || output_cursor != output.bytes.size()) {
      add_issue(
          result, WasmAssetStoreIssueCode::InvalidResponse,
          "portable asset response batch has trailing bytes"
      );
      return result;
    }
