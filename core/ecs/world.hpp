...
inline Entity make_entity(std::uint32_t idx, std::uint32_t gen) {
    return (static_cast<Entity>(gen) << 32) | idx;
}

namespace detail {
inline std::size_t next_component_id() { static std::size_t c = 0; return c++; }
template <class T> std::size_t component_id() { static const std::size_t id = next_component_id(); return id; }
}  // namespace detail
...
public:
    Entity create() {
        ...
        else { idx = static_cast<std::uint32_t>(generations_.size()); generations_.push_back(0); }
    ...
    void destroy(Entity e) {
        if (!valid(e)) return;
        const std::uint32_t i = entity_index(e);
        for (auto& p : pools_) if (p && p->contains(i)) p->remove(i);  // type-erased
    ...
    T& add(Entity e, Args&&... args) {
        return storage<T>().emplace(entity_index(e), std::forward<Args>(args)...);
    }
