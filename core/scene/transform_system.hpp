...
public:
    void rebuild(ecs::World& w) {
        ...
        std::stable_sort(ents.begin(), ents.end(), [&](ecs::Entity a, ecs::Entity b) {
            return depth[ecs::entity_index(a)] < depth[ecs::entity_index(b)];
        });

        std::unordered_map<std::uint32_t, int> pos;
        pos.reserve(ents.size());
        for (int i = 0; i < static_cast<int>(ents.size()); ++i)
            pos[ecs::entity_index(ents[i])] = i;
