...
int main() {
    ...
    for (std::size_t i = 0; i < N; ++i) {
        TransformStore::Id id =
            store.create(i > 0 ? static_cast<TransformStore::Id>((i - 1) / 2) : TransformStore::npos);
    ...
    const bool matchL = sampleR[3][0] == sampleL[3][0] && sampleR[3][1] == sampleL[3][1] &&
                        sampleR[3][2] == sampleL[3][2];
    const bool matchS = sampleR[3][0] == sampleS[3][0] && sampleR[3][1] == sampleS[3][1] &&
                        sampleR[3][2] == sampleS[3][2];
    ...
    std::printf("SoA store + SIMD update: %.2f ms  (%.2fx vs recursive, %.2fx vs linearized)  [%s]\n",
                bestS, bestR / bestS, bestL / bestS, matchS ? "match" : "MISMATCH");
    return 0;
}
