...
template <class T>
void convert_typed(const char* raw, double slope, double intercept, std::size_t n,
                   uint16_t* dst, int& vmin, int& vmax) {
...
void convert_slice(const char* raw, const gdcm::PixelFormat& pf, double slope,
                   double intercept, std::size_t n, uint16_t* dst, int& vmin, int& vmax) {
    switch (pf.GetScalarType()) {
        case gdcm::PixelFormat::UINT8:
            convert_typed<uint8_t>(raw, slope, intercept, n, dst, vmin, vmax); break;
        case gdcm::PixelFormat::INT8:
            convert_typed<int8_t>(raw, slope, intercept, n, dst, vmin, vmax); break;
        case gdcm::PixelFormat::UINT16:
            convert_typed<uint16_t>(raw, slope, intercept, n, dst, vmin, vmax); break;
        case gdcm::PixelFormat::INT16:
            convert_typed<int16_t>(raw, slope, intercept, n, dst, vmin, vmax); break;
        default:
...
extern "C" VolumeData volume_io_load_series(const char* dir) {
    ...
    const bool have_window =
        read_ds(ds0, 0x0028, 0x1050, wc) && read_ds(ds0, 0x0028, 0x1051, ww);
    ...
    for (unsigned int z = 0; z < depth; ++z) {
        ...
        if (!r.Read()) { std::free(vox); return v; }
        const gdcm::Image& im = r.GetImage();
        buf.resize(im.GetBufferLength());
        if (!im.GetBuffer(buf.data())) { std::free(vox); return v; }
        convert_slice(buf.data(), im.GetPixelFormat(), im.GetSlope(), im.GetIntercept(),
                      slice, vox + static_cast<std::size_t>(z) * slice, vmin, vmax);
    }
