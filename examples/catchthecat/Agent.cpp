...
std::vector<Point2D> Agent::generatePath(World* w) {
  ...
  int currentCost = 0;
  // bootstrap state
  Point2DPrioritized catPos = {w->getCat(), 0};
  ...
  while (!frontier.empty()) {
    ...
    vector<Point2D> neighs = getVisitableNeighbors(w, current.point, visited);
    ...
    for (int i = 0;i<neighs.size();i++) {
      int nextCost = currentCost + heuristicManhattan(neighs[i], current.point);
      if (cameFrom.find(neighs[i]) == cameFrom.end() || nextCost < current.priority) {
        currentCost +=nextCost;
        cameFrom[neighs[i]] = current.point;
        Point2DPrioritized currentNeigh = {current.point, currentCost};
